//! MCP 2026-07-28 roots example server.
//!
//! Under MCP 2026-07-28 there is no `roots/list` server->client *request*: the
...
#[tokio::main]
async fn main() {
    // neva does not install a global subscriber here; do it so the per-round log
    // above is visible.
    ...
    App::new()
        ...
        .with_options(|opt| {
            opt.with_name("Roots Example Server")
